...
class Board:
    ...
    def move(self,from_col, from_row, to_col, to_row):
        if self.board[from_col][7-from_row].piece.try_movement(to_col, to_row):
            ...
            if self.board[from_col][7-from_row].piece.colour == "w":
                self.board[from_col][7-from_row].piece.cells_attacked(self.board_white_attack, from_col, from_row,"stop")
            elif self.board[from_col][7-from_row].piece.colour == "b":
                self.board[from_col][7-from_row].piece.cells_attacked(self.board_black_attack, from_col, from_row,"stop")

            self.board[from_col][7-from_row].piece.movement(to_col, to_row)

            if self.board[to_col][7-to_row].piece.colour == "w":
                self.board[to_col][7-to_row].piece.cells_attacked(self.board_white_attack, to_col, to_row,"attack")
            elif self.board[to_col][7-to_row].piece.colour == "b":
                self.board[to_col][7-to_row].piece.cells_attacked(self.board_black_attack, to_col, to_row,"attack")
    ...
    def __repr__(self):
        ...
        for row in range(0,8):
            for col in range(0,8):
                board += get_piece_initials(self.board[col][row]).ljust(3)
                board_attack += "%s%s".ljust(5) % (self.board_white_attack[7-row][col], self.board_black_attack[7-row][col])
            board += "\n"
            board_attack += "\n"
        board += "\n"
        board_attack += "\n"
